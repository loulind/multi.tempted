...
`multi.tempted` is the package itself; the rest are used only for the plots in
this walkthrough.
...
omic modalities (**cytokine**, **metabolome**, **lipid**, **protein**), each on
a log₁₀ scale, measured at five visits. `ipop` also carries subject metadata
(`meta_subj`) and per-sample `SubjectID` / `timepoint` (`meta_visit`).
